...
class HookTests(unittest.TestCase):
    def setUp(self):
        ...
        self.git('init', '-q')
        for path, content in {
            'site.json': '{}', 'index.html': 'valid staged export', 'build-static.sh': '#!/bin/sh\n',
            'vendor/logseq-static-garden/static-garden/requirements.txt': '', 'branding/logo.png': 'logo',
            'branding/logo.svg': 'logo', 'vendor/logseq-static-garden/vendor/katex/katex.min.js': 'renderer',
            'assets/example.txt': 'original', 'README.md': 'read me',
        }.items():
            self.write(path, content)
        (self.root / 'tools').mkdir()
        self.write('vendor/logseq-static-garden/static-garden/build.py', '''from pathlib import Path
import sys
...
''')
        shutil.copy2(HERE / 'exporter.py', self.root / 'tools/exporter.py')
        self.pin()
        self.git('add', '.')
    ...
    def pin(self):
        from exporter import manifest
        import json
        self.write('exporter.lock.json', json.dumps({'commit': 'fixture', 'files': manifest(self.root / 'vendor/logseq-static-garden')}))
    ...
    def test_partial_staging_uses_staged_export_and_assets(self):
        self.write('site.json', '{"changed": true}'); self.git('add', 'site.json')
        self.write('index.html', 'broken unstaged export')
    ...
    def test_staged_deletion_is_checked(self):
        self.git('rm', '--cached', 'vendor/logseq-static-garden/vendor/katex/katex.min.js')
        result = self.run_hook()
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('Exporter snapshot differs', result.stderr)
        self.assertTrue((self.root/'vendor/logseq-static-garden/vendor/katex/katex.min.js').exists())

    def test_missing_dependencies_do_not_trigger_install(self):
        self.write('vendor/logseq-static-garden/static-garden/requirements.txt', 'this-package-does-not-exist==1.0\n'); self.pin(); self.git('add', 'vendor', 'exporter.lock.json')
        result = self.run_hook()
        self.assertNotEqual(result.returncode, 0)
